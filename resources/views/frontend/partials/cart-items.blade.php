        ...
        <div class="item flex items-center justify-between gap-3 pb-5 border-b border-line mb-5">
            ...
            <div class="flex flex-col gap-2 items-end">
                <div class="quantity-block py-1 px-1 flex items-center justify-between rounded-full border border-gray-200 w-28 bg-gray-50 shadow-sm">
                    <i class="ph-bold ph-minus text-sm w-8 h-8 flex items-center justify-center rounded-full bg-white shadow-sm hover:bg-black hover:text-white transition-colors duration-300 cursor-pointer update-cart-btn"
                       data-key="{{ $key }}" data-qty="{{ $item['quantity'] - 1 }}"></i>
                    <div class="quantity font-semibold text-sm">{{ $item['quantity'] }}</div>
                    <i class="ph-bold ph-plus text-sm w-8 h-8 flex items-center justify-center rounded-full bg-white shadow-sm hover:bg-black hover:text-white transition-colors duration-300 cursor-pointer update-cart-btn"
                       data-key="{{ $key }}" data-qty="{{ $item['quantity'] + 1 }}"></i>
                </div>
